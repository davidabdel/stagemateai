  ...
  try {
    ...
    switch (event.type) {
      ...
      case 'customer.subscription.updated': {
        ...
        // Check if this update is a cancellation (status changed to canceled)
        if (subscription.status === 'canceled' && 
            previousAttributes && 
            previousAttributes.status && 
            previousAttributes.status !== 'canceled') {
          console.log('Subscription canceled via status update:', subscription.id);
...
// Handle subscription marked for cancellation at period end
async function handleSubscriptionMarkedForCancellation(customerId: string, subscriptionId: string) {
  try {
    console.log(`Processing subscription marked for cancellation: ${subscriptionId} for customer ${customerId}`);
    ...
    // Update subscription status in the database to 'canceled' but keep the plan active
    // First, check if we have a subscriptions table
    const { error: tableCheckError } = await supabase
      .from('subscriptions')
      .select('id')
    ...
    console.log(`Successfully marked subscription as canceled for user ${userId}. Will remain active until ${currentPeriodEnd.toISOString()}`);
  } catch (error) {
    console.error('Error handling subscription marked for cancellation:', error);
  }
}
...
async function handleSubscriptionCancelled(customerId: string, subscriptionId: string) {
  try {
    ...
    // Try to get the specific subscription that was canceled
    let subscription;
    try {
      ...
      // Fall back to getting the latest subscription for this customer
      ...
      if (subscriptions.data.length > 0) {
        ...
        console.error(`No canceled subscriptions found for customer ${customerId}`);
        return;
      }
    }
